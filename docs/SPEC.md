...
`feedback_tombstones` rows are not deleted by cleanup.
...
| `serve_radar.py` | Opt-in localhost live radar (GET `/`, `/api/state`, `/api/feedback`; POST `/api/feedback`); loopback by default; unauthenticated. Never writes `data/radar/`. |
